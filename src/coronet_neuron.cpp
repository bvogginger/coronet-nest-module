...
nest::RecordablesMap<mynest::coronet_neuron> mynest::coronet_neuron::recordablesMap_;
...
namespace nest  // template specialization must be placed in namespace
{
  ...
  template <>
  void RecordablesMap<mynest::coronet_neuron>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, 
	    &mynest::coronet_neuron::get_y_elem_<mynest::coronet_neuron::State_::V_M>);
    insert_(names::g_ex, 
	    &mynest::coronet_neuron::get_y_elem_<mynest::coronet_neuron::State_::G_EXC>);
    insert_(names::g_in, 
	    &mynest::coronet_neuron::get_y_elem_<mynest::coronet_neuron::State_::G_INH>);
  }
}

extern "C"
inline int mynest::iaf_cond_exp_dynamics(double, const double y[], double f[], void* pnode)
{ 
  // a shorthand
  typedef mynest::coronet_neuron::State_ S;

  // get access to node so we can almost work as in a member function
  assert(pnode);
  const mynest::coronet_neuron& node =  *(reinterpret_cast<mynest::coronet_neuron*>(pnode));
...
mynest::coronet_neuron::Parameters_::Parameters_()
  : V_th_      (-55.0    ),  // mV
    V_reset_   (-60.0    ),  // mV
    t_ref_     (  2.0    ),  // ms
    g_L        ( 16.6667 ),  // nS
...
mynest::coronet_neuron::State_::State_(const Parameters_& p)
  : r_(0)
{
  y_[V_M] = p.E_L;
  y_[G_EXC] = y_[G_INH] = 0;
...
mynest::coronet_neuron::State_::State_(const State_& s)
  : r_(s.r_)
{
...
mynest::coronet_neuron::State_& mynest::coronet_neuron::State_::operator=(const State_& s)
{
...
void mynest::coronet_neuron::Parameters_::get(DictionaryDatum &d) const
{
...
void mynest::coronet_neuron::Parameters_::set(const DictionaryDatum& d)
{
...
void mynest::coronet_neuron::State_::get(DictionaryDatum &d) const
{
  def<double>(d, names::V_m, y_[V_M]); // Membrane potential
}

void mynest::coronet_neuron::State_::set(const DictionaryDatum& d, const Parameters_&)
{
...
mynest::coronet_neuron::coronet_neuron()
  : Archiving_Node(), 
    P_(), 
    S_(P_),
    B_(*this)
...
mynest::coronet_neuron::coronet_neuron(const coronet_neuron& n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
...
void mynest::coronet_neuron::init_state_(const Node& proto)
{
  const coronet_neuron& pr = downcast<coronet_neuron>(proto);
  S_ = pr.S_;
}
...
void mynest::coronet_neuron::calibrate()
{
...
void mynest::coronet_neuron::update(Time const & origin, const long_t from, const long_t to)
{
...
void mynest::coronet_neuron::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
